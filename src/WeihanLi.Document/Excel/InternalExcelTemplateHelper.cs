    ...
    internal class InternalExcelTemplateHelper
    {
            ...
            object extraData = null)
        {
            ...
            var globalDictionary = extraData.ParseParamInfo()
                .ToDictionary(x => TemplateHelper.TemplateOptions.TemplateGlobalParamFormat.FormatWith(x.Key), x => x.Value);
            foreach (var propertyConfiguration in propertyColumnDictionary)
            {
                globalDictionary.Add(TemplateHelper.TemplateOptions.TemplateHeaderParamFormat.FormatWith(propertyConfiguration.Key.Name), propertyConfiguration.Value.ColumnTitle);
            }

            var dataFuncDictionary = propertyColumnDictionary
                .ToDictionary(x => TemplateHelper.TemplateOptions.TemplateDataParamFormat.FormatWith(x.Key.Name), x => x.Key.GetValueGetter<TEntity>());
            foreach (var key in propertyColumnDictionary.Keys)
            {
                ...
                {
                    dataFuncDictionary[TemplateHelper.TemplateOptions.TemplateDataParamFormat.FormatWith(key.Name)] = entity =>
                    {
                ...
                for (var cellIndex = row.FirstCellNum; cellIndex < row.LastCellNum; cellIndex++)
                {
                    ...
                    if (!string.IsNullOrEmpty(cellValue))
                    {
                        ...
                        if (dataStartRow <= 0 || dataRowsCount <= 0)
                        {
                            if (dataStartRow >= 0)
                            {
                                if (cellValue.Contains(TemplateHelper.TemplateOptions.TemplateDataEnd))
                                {
                                    dataRowsCount = rowIndex - dataStartRow + 1;
                                    cellValue = cellValue.Replace(TemplateHelper.TemplateOptions.TemplateDataEnd, string.Empty);
                                }
                            }
                            else
                            {
                                if (cellValue.Contains(TemplateHelper.TemplateOptions.TemplateDataBegin))
                                {
                                    dataStartRow = rowIndex;
                                    cellValue = cellValue.Replace(TemplateHelper.TemplateOptions.TemplateDataBegin, string.Empty);
                                }
                            }
                        }
                    ...
                    for (var i = 0; i < dataRowsCount; i++)
                    {
                        ...
                        if (null != row)
                        {
                            for (var j = 0; j < row.LastCellNum; j++)
                            {
                                ...
                                {
                                    var cellValue = cell.GetCellValue<string>();
                                    if (!string.IsNullOrEmpty(cellValue) && cellValue.Contains(TemplateHelper.TemplateOptions.TemplateDataPrefix))
                                    {
                                        var beforeValue = cellValue;
